...
test("action effects apply their chip, multiplier, and final-score stages", () => {
  ...
  assert.equal(voidErosion.score, 332);
  ...
  assert.equal(targeting.score, 424);
  ...
  assert.equal(vigorous.score, 222);
});
...
test("tomato effects use throws for Old days and the restored x5 final bonuses", () => {
  ...
  assert.equal(shooter.score, 161);
  ...
  assert.equal(oldDays.score, 123);
  ...
  assert.equal(tempered.score, 460);
});
